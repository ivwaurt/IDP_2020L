...
//Msg to update motor speed
void msgMotor(uint8_t msg){
  moveFast = (bitRead(msg,2) == 0) && (bitRead(msg,0) == 0) ;
  //0th bit = move/not move, 1st bit = reverse
  motor_L( (bitRead(msg,3) ? ( moveFast ? v_f : v ) : 0) * (bitRead(msg,2) ? -1 : 1) );
  //2nd bit = move/not move, 3rd bit = reverse
  motor_R( (bitRead(msg,1) ? ( moveFast ? v_f : v ) : 0) * (bitRead(msg,0) ? -1 : 1) );
}
